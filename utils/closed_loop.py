...
def run_simulation(num_steps: int, 
    ...
    for step in range(num_steps):
        ...
        if reset_game:
            ...
            state = make_initial_state() if state is None else state
            max_size = max(a.size for a in state.asteroids)
...
def _draw_ship(ax, ship: Ship):
    ...
    ax.fill(pts_world[:,0], pts_world[:,1], edgecolor="blue", facecolor="blue", alpha=0.7, linewidth=1.0)
...
def animate_history(history: List[StepRecord],
    ...
    def update(frame_idx):
        ...
        if rec.hit:
            ...
            ax.text(rec.ship.x, rec.ship.y, "", color="red", fontsize=14,
                    ha="center", va="center", fontweight="bold")

        # Asteroids
        for a in rec.asteroids:
            circ = plt.Circle((a.x, a.y), radius=a.size,
                              edgecolor="gray", facecolor="gray", linewidth=1.0, alpha=0.5)
            ax.add_patch(circ)
    ...
    anim = FuncAnimation(fig, update, frames=len(history), init_func=init, interval=1000 * DT,  # ms per frame
        blit=False,
    )

    if save_path is not None:
        # If ffmpeg is not available, Matplotlib uses PillowWriter,
        # which cannot write MP4 files. Convert .mp4 to .gif.
